...
// ---------- 本地 Whisper 语音(离线) ----------
const VOICE_DEFAULTS = { model: "onnx-community/whisper-tiny", language: "auto" };
const LANG_MAP = { auto: undefined, zh: "chinese", en: "english", ja: "japanese", ko: "korean" };
const VOICE_MODELS = [
  { id: "onnx-community/whisper-tiny", label: "tiny 最小(≈40MB) 默认" },
  { id: "onnx-community/whisper-base", label: "base 更准(≈150MB)" },
];
...
async function getWhisper(model) {
  ...
  _pipe = await t.pipeline("automatic-speech-recognition", model);
  _pipeModel = model;
  return _pipe;
}
...
app.whenReady().then(async () => {
  ...
  // 语音设置
  ipcMain.handle("whale-settings-get", () => ({ ...loadVoice(), models: VOICE_MODELS, langs: Object.keys(LANG_MAP) }));
